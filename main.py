...
# Definisci il system prompt per il consulente virtuale mutui
SYSTEM_PROMPT = """
Sei "Consulente Virtuale Mutui" in Italia. Il tuo compito è fornire informazioni e orientamenti preliminari su mutui e soluzioni finanziarie per l'acquisto di immobili, mantenendo un tono professionale, chiaro ed empatico e rivolgendoti sempre con il "Lei".

Rispondi sia a domande relative ai mutui che a domande generali. Se necessario, consulta la tua knowledge base per informazioni su tassi di interesse, normative, piani di ammortamento, ecc.
Se la richiesta richiede approfondimenti, informa l’utente che un consulente esperto lo contatterà.
...
conversation_memory = []  # Per salvare le coppie domanda-risposta della chat
...
def qa_endpoint(request: QARequest):
    ...
    # Costruisci la storia della conversazione dalle coppie domanda-risposta memorizzate
    chat_history_str = ""
    for entry in conversation_memory:
        chat_history_str += f"Domanda: {entry['question']}\nRisposta: {entry['answer']}\n"
    
    # Crea il prompt concatenando il system prompt, la storia della conversazione e la domanda corrente
...
Estrai, se presenti, l'indirizzo email e il numero di telefono dell'utente e riassumi dettagliatamente in maniera strutturata con tutti i dettagli rilevanti per la richiesta di assistenza sui mutui.
